...
Answer: We can  use C++ to rewrite the code to get huge improvement, the following result using the same data but with C++ implementation.
```{r}
system.time(brute_force_knapsack(x = knapsack_objects[1:8,], W = 3500, fast = TRUE))
```